...
class LlavaMetaForCausalLM(ABC):
    ...
    def encode_rgbd_videos(self, images, depths, poses, intrinsics, lengths=None):
        ...
        # CLS→patch attention importance: (B*V, 576) → (B, V, 576)
        cls_attn = getattr(vision_tower, '_cls_attn', None)
        if cls_attn is not None:
            cls_attn = cls_attn.reshape(batch_size, num_view, -1)
        ...
        video_features, batch_offset = self.get_model().get_video_tower()(image_features, depths, poses, intrinsics, lengths=lengths, cls_attn=cls_attn)  # (B, 1024, C)
            ...
            for i in range(num_specials + 1):  # num_images = 1? [0, 1]
                ...
                if i < num_specials:
                    ...
                    if special_token == IMAGE_TOKEN_INDEX:
                        ...
                        cur_image_idx += 1
                        # Track image token position for FastV
                        _fastv_img_start = sum(x.shape[0] for x in cur_new_input_embeds)
                        _fastv_img_len = cur_image_features.shape[0]
                        try:
                            from transformers.models.llama.modeling_llama import FASTV_STATE
                            FASTV_STATE["img_start"] = _fastv_img_start
                            FASTV_STATE["img_len"] = _fastv_img_len
