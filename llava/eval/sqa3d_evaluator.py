...
def calc_sqa3d_score(preds, gts):
    ...
    for pred, gt in tqdm(zip(preds, gts)):
        assert pred['question_id'] == gt['question_id']

        pred_answer = clean_answer(pred['text'])
        gt_answers = [clean_answer(gt['text'])]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pred-file', type=str, default='llava-3d-7b-sqa3d_test_answer.json')
    parser.add_argument('--gt-file', type=str, default='playground/data/annotations/llava3d_sqa3d_test_answer.json')
    args = parser.parse_args()

    preds = [json.loads(q) for q in open(args.pred_file, "r")]
    gts = mmengine.load(args.gt_file)

    # Match by question_id (handles partial results)
    pred_dict = {p['question_id']: p for p in preds}
    matched_preds = []
    matched_gts = []
    for gt in gts:
        if gt['question_id'] in pred_dict:
            matched_preds.append(pred_dict[gt['question_id']])
            matched_gts.append(gt)

    print(f"Matched {len(matched_preds)}/{len(gts)} (pred file: {len(preds)})")
    val_scores = calc_sqa3d_score(matched_preds, matched_gts)
    print()
