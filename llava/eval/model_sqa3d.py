...
def eval_model(args):
    ...
    for line in pbar:
        ...
        if args.visualize:
            ...
            # voxel xyz
            video_tower = model.get_model().get_video_tower()
            voxel_xyz = video_tower._voxel_xyz[0].cpu().float()
            ...
            # Mid-layer (16th) attention
            last_attn = fwd_out.attentions[15].float()
            last_attn = last_attn.mean(dim=1)
            query_to_vis = last_attn[0, vis_end:, vis_start:vis_end]
            avg_attn = query_to_vis.mean(dim=0).cpu().numpy()
            ...
            ax1.set_xlabel('Voxel Index'); ax1.set_ylabel('LLM Attention')
            ...
            fig_html.add_trace(go.Scatter3d(
                ...
                mode='markers', marker=dict(size=1.2, color=rgb_str, opacity=0.7),
                name='RGB',
            ), row=1, col=1)
            fig_html.add_trace(go.Scatter3d(
                x=xyz[:, 0], y=xyz[:, 1], z=xyz[:, 2],
                mode='markers',
                marker=dict(size=3, color=avg_attn, colorscale='Hot', opacity=0.85,
                            colorbar=dict(title='Attn', x=1.0, len=0.6), cmin=0),
                text=[f'vox {i}<br>attn={avg_attn[i]:.6f}' for i in range(n_visual)],
                hoverinfo='text', name='Attention',
            ), row=1, col=2)
            scene_cfg = dict(aspectmode='data')
            fig_html.update_layout(
                title=f'[{idx}] Q: {cur_prompt}<br>A: {outputs}',
                width=1600, height=700,
                scene=scene_cfg, scene2=scene_cfg, showlegend=False,
            )
            html_path = os.path.join(vis_dir, f'{vis_prefix}_{idx}.html')
            fig_html.write_html(html_path)
            print(f"[VIS] {save_path}  [HTML] {html_path}  (n_vox={n_visual})")
    ...
    ans_file.close()
    if skipped:
        print(f"\n[INFO] Skipped {skipped}/{len(questions)} questions (missing scenes)")
